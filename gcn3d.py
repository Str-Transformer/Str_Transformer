...
def relu_average_pooling(x):
    x_clamped = x.clamp(min=1e-6)
    out = torch.mean(x_clamped, dim=2)
...
class Pool_layer(nn.Module):
    def __init__(self, pooling_rate: int= 4, neighbor_num: int=  4, pooling_type = 'Max'):
        super().__init__()
        self.pooling_rate = pooling_rate
        self.neighbor_num = neighbor_num
        self.pooling_type = pooling_type
        
    def FPS(self,points, feature_map, num_points = 256, num_group = 64):
        B, N, D = points.size()
        # batch_flatten_points = points.view(B*N, D)
        points = points.contiguous()
        flattend_points = points.view(B*N, D)
        batch = torch.arange(B, device='cuda').repeat_interleave(N)
        # print(batch.device, flattend_points.device, points.device)
        
        sampled_idx = fps(points.view(B*N, D), batch=batch, ratio=num_group/num_points, random_start=True).view(B, -1)
        # shape : [B, G=16, D]
        sampled_points = flattend_points[sampled_idx, :].view(B, -1, D)
        
        _, _, feature_size = feature_map.size()
        feature_map = feature_map.contiguous()
        flattend_feature_map = feature_map.view(B*N, feature_size)
        sampled_feature_map = flattend_feature_map[sampled_idx, :].view(B, -1, feature_size)
        return sampled_points, sampled_feature_map
    
    
    def voxelization(self, pivot_points, points, num_points_in_group=16):
        B, N, D = points.size()
        k=num_points_in_group - 1
        dists = torch.cdist(pivot_points, points)
        _, knn_idx = torch.topk(dists, k, dim=-1, largest=False, sorted=True)
        # shpae : [B, G=16, K=15, D]
        knn_points = points[torch.arange(B).unsqueeze(1).unsqueeze(2), knn_idx, :]
        # Shape : [B, G=16, N=16, D]
        grouped_points = torch.cat([pivot_points.unsqueeze(2), knn_points], dim=2)
    ...
    def forward(self, 
        ...
        if self.pooling_type == 'Max':
            ...
            pooled_feature = torch.mean(neighbor_feature, dim= 2)
        elif self.pooling_type == 'Relu_pooling':
            pooled_feature = relu_average_pooling(neighbor_feature)

        pool_num = int(vertice_num / self.pooling_rate)
        vertices_pool, feature_map_pool = self.FPS(vertices, pooled_feature, num_points=vertice_num, num_group=pool_num)
